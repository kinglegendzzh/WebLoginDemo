...
//登录处理程序
	...
	$rememberpassword = isset($_POST['rememberpassword'])?$_POST['rememberpassword']:NULL;
    //判断用户名和密码是否为空
    if(!empty($username)&&!empty($password)) {
        //建立连接
        $conn = mysqli_connect('localhost','root','wxhzzh','login');
        //准备SQL语句
        ...
        if($username==$row['username']&&$password==$row['password']) {
			...
			if($rememberpassword=="on"){
				setcookie("mima", $password, time()+7*24*3600);
			}
            //开启session
            session_start();
            //创建session
